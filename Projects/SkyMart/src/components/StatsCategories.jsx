...
const StatsCategories = () => {
  ...
  const stats = useMemo(() => [
    {
      icon: Package,
      color: "bg-[#fff700]/10 text-[#fff700]",
      value: `${cart.length}`,
      title: "Cart Items",
  ...
  return (
    <section className="max-w-[1650px] mx-auto px-5 mt-8">
      ...
      <div className="grid lg:grid-cols-4 md:grid-cols-2 gap-4">
        {stats.map((item, index) => {
          ...
          return (
            <div
              key={index}
              className="rounded-[26px] border border-zinc-700 bg-[#111111] px-6 py-6 hover:border-[#fff700]/40 transition"
            >
      ...
      <div className="mt-10 mb-6 flex items-center justify-between">
        ...
        <Link
        to="/shop"
        className="flex items-center gap-2 text-[#fff700] text-lg font-semibold hover:gap-3 transition-all"
      >
        View All
        <ArrowRight size={18} />
      </Link>
    ...
    categories.map((category, index) => {
      ...
      return (
        ...
        >
          ...
          <h3 className="mt-3 text-xl font-semibold group-hover:text-yellow-500 capitalize">
            {category.name}
          </h3>
