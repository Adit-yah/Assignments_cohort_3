    ...
    <article className="product-card rounded-[24px] border border-zinc-700 bg-zinc-950 p-4 shadow-lg shadow-black/30">
      ...
      <div className="product-card__content flex flex-col gap-3">
        <div className="flex items-center justify-between gap-3">
          ...
          <span className="text-sm uppercase tracking-[0.12em] text-[#fff700]">{category}</span>
        </div>
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          {onAddToCart && (
            <button
              className="rounded-2xl bg-[#fff700] px-3 py-2 text-sm font-semibold text-black transition hover:bg-yellow-300"
              onClick={onAddToCart}
            >
              Add to cart
            </button>
          ...
          {onEdit && (
            <button
              className="rounded-2xl border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-white transition hover:border-[#fff700]"
              onClick={onEdit}
            >
              Increase price
            </button>
