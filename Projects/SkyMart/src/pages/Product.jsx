      ...
      <main className="min-h-screen bg-[#0b0b0b] text-white">
        <div className="max-w-[1000px] mx-auto px-5 py-16 text-center">
          ...
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="mt-6 rounded-2xl border border-zinc-700 bg-[#111111] px-6 py-3 text-sm font-semibold text-white transition hover:border-[#fff700]"
          >
            Go back
          </button>
        </div>
        ...
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={() => navigate(-1)}
              className="inline-flex items-center gap-2 rounded-2xl border border-zinc-700 bg-[#111111] px-4 py-2 text-sm text-zinc-300 transition hover:border-[#fff700]"
            >
              <ArrowLeft size={18} />
              Back
            </button>
          ...
          <button
            type="button"
            onClick={handleNext}
            className="inline-flex items-center gap-2 rounded-2xl border border-zinc-700 bg-[#111111] px-4 py-2 text-sm text-white transition hover:border-[#fff700]"
          >
            Next product
            <ArrowRight size={18} />
          </button>
        ...
        <div className="grid gap-8 lg:grid-cols-[420px_1fr]">
          <div className="rounded-[28px] border border-zinc-700 bg-[#111111] p-6 text-center">
            <div className="mb-4 flex items-center justify-between rounded-2xl border border-zinc-800 bg-zinc-950/70 px-4 py-3 text-sm text-zinc-400">
              <span className="inline-flex items-center gap-2">
                <Package size={16} className="text-[#fff700]" />
                Premium product
              </span>
          ...
          <div className="space-y-6">
            <div className="rounded-[28px] border border-zinc-700 bg-[#111111] p-6">
              <p className="text-sm uppercase tracking-[0.3em] text-[#fff700]">{product.category}</p>
              ...
              <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <span className="text-4xl font-bold text-[#fff700]">${product.price}</span>
                </div>
                <div className="flex flex-wrap gap-3">
                  <button
                    type="button"
                    onClick={() => navigate('/shop')}
                    className="inline-flex items-center gap-2 rounded-2xl border border-zinc-700 bg-zinc-900 px-5 py-3 text-sm font-semibold text-white transition hover:border-[#fff700]"
                  >
                    Continue shopping
                  </button>
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-2xl bg-[#fff700] px-6 py-3 text-sm font-semibold text-black transition hover:bg-yellow-300"
                  >
              ...
              {notification ? (
                <div className="mt-6 rounded-3xl border border-yellow-500 bg-[#fff700]/10 px-4 py-3 text-sm text-yellow-200">
                  {notification}
                </div>
              ) : null}
            </div>
            ...
            <div className="rounded-[28px] border border-zinc-700 bg-[#111111] p-6">
              <div className="mb-4 flex items-center justify-between">
                ...
                <Link to="/shop" className="text-sm font-semibold text-[#fff700] hover:text-white">
                  View all
                </Link>
              </div>
                ...
                {relatedProducts.length > 0 ? (
                  relatedProducts.map((item) => (
                    <button
                      ...
                      className="group rounded-3xl border border-zinc-700 bg-zinc-950 p-4 text-left transition hover:border-[#fff700]"
                    >
                      <img
                        src={item.image}
                        alt={item.title}
